...
let _c=Lazy((_i) => class Task_1 extends Object {
  static {
    _c=_i(this);
  }
  action;
  ...
  static Run(func, ct){
    const task=func();
    return ct.c?_c.FromCanceled(ct):(task.Status===0?task.Start():void 0,task);
  }
  static Run_1(func, ct){
    const res=new Task("New_4", func, ct, 0, null, void 0);
    res.Start();
    return res;
  }
  static Run_2(func, ct){
    const task=func();
    return ct.c?_c.FromCanceled_1(ct):(task.Status===0?task.Start():void 0,task);
  }
  static Run_3(action, ct){
    const res=new Task_1("New_4", action, ct, 0, null);
    res.Start();
    return res;
  }
  static FromResult(res){
    return new Task("New_4", null, noneCT(), 5, null, res);
  }
  static FromException(exc){
    return new Task("New_4", null, noneCT(), 7, new AggregateException("New_3", [exc]), null);
  }
  static FromException_1(exc){
    return new Task_1("New_4", null, noneCT(), 7, new AggregateException("New_3", [exc]));
  }
  static FromCanceled(ct){
    return new Task("New_4", null, ct, 6, new AggregateException("New_3", [new TaskCanceledException("New")]), null);
  }
  static FromCanceled_1(ct){
    return new Task_1("New_4", null, ct, 6, new AggregateException("New_3", [new TaskCanceledException("New")]));
  }
  Start(){
    if(this.status===0){
      this.status=2;
